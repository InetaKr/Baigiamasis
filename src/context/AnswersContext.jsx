...
const AnswersProvider = ({ children }) => {
    ...
    await fetch(`http://localhost:5000/answers/${id}`, {
      ...
      if (res.ok) {
        setAnswers(
          answers.map((answer) =>
            answer.id === id
              ? { ...answer, answer: updatedAnswer.answer, ...updatedAnswer }
              : answer
          )
        );
      }
  ...
  const handleLike = async (id) => {
    ...
    if (!updatedAnswer.likedBy.includes(loggedInUser.id)) {
      ...
      updatedAnswer.disLikedBy = updatedAnswer.disLikedBy.filter(
        (userId) => userId !== loggedInUser.id
      );
    } else {
      updatedAnswer.likedBy = updatedAnswer.likedBy.filter(
        (userId) => userId !== loggedInUser.id
      );
    }
